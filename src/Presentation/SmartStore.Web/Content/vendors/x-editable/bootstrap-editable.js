...
(function ($) {
	...
	var Constructor = function (options) {
		...
		if (!options.select2.tags && options.source) {
			...
			if (typeof source === 'string') {
				var ajax = options.select2.ajax = options.select2.ajax || {};
				//some default ajax params
				if (!ajax.data) {
					ajax.data = function (term) { return { term: term }; };
				}
				if (!ajax.processResults) {
					ajax.processResults = function (data) { return { results: data.items }; };
				}
				ajax.url = source;
			}
			else {
	...
	$.extend(Constructor.prototype, {
		...
		value2html: function (value, element) {
			...
			if ($.isArray(data)) {
				...
				text = [];
				$.each(data, function (k, v) {
					text.push(v && typeof v === 'object' ? that.templateSelection(v) : v);
				});
			}
			else if (data) {
				text = that.templateSelection(data);
			}
		...
		value2input: function (value) {
			...
			if (this.isRemote && !this.isMultiple && !this.options.select2.initSelection) {
				...
				var customId = this.options.select2.id,
					customText = this.options.select2.templateSelection;
...
(function ($) {
	...
	$.extend(DateTime.prototype, {
		...
		value2html: function (value, element) {
			...
			if (value) {
				var m = moment(value);
				text = m.format(this.options.datetimepicker.format);
			}
		...
		input2value: function () {
			...
			if (picker.datetimepicker('date')) {
				// Use 'viewDate' (not 'date') to get current date from input (!) control.
				var m = picker.datetimepicker('viewDate');
				if (m != null) {
					return m.toDate();
				}
			}
